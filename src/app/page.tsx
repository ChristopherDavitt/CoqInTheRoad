      ...
      if (window.ethereum && account.address) {
        try {
          ...
          gameContract.on(filter, (sender, won, coqWon) => {
            // Your event handling logic
            dispatch({ type: 'UPDATE_WIN', payload: won });
        ...
        <HStack gap="1rem" align="center" justify="center">
          ...
          <Stack bg="bg" mt={8} mb='2rem' gap='0.5rem' borderRadius={6} border="solid 1px" borderColor="gray.100" padding={4}>
              ...
              <Flex width="100%" align='center' justify='space-between'>
                ...
                <HStack flex={1} spacing={1} align='center' justify='center'>
                ...
                <Text textAlign="right" flex={1}>{cars} CARS</Text>
              </Flex>
              <Divider />
              {/* Lane */}
              <Flex align='center' justify='space-between' gap='0.5rem'>
                <Heading flex={1} size='sm'>Lane</Heading>
                <HStack flex={1} spacing={1} align='center' justify='space-between'>
                ...
                <Text textAlign={"right"}>{lane} Lane #</Text>
              </Flex>
              <Divider />
              <Flex justifyContent="space-between" width="100%" align="center">
                ...
                <Button py={0} size="sm" alignSelf="flex-end" onClick={() => setCoqBet(69000000)}>max</Button>
              </Flex>
              <NumberInput 
                max={100000000}
                size='lg'
            ...
            {Number(allowance) > 190000000000 ?
              <Button isDisabled={loading || coqBet < Number(minBet) || (coqBet * carMultipliers[cars - 1] / 1000) < (Number(treasuryBalance) / 20)} onClick={() => playGame(String(coqBet), lane, cars)}>
                Play Game
              </Button>
            : <Button onClick={() => approve('200000000000')}>Approve</Button>}
            <Text align='center' color='text' fontSize='12px' lineHeight='16px'>
              Contract is paused
            </Text>
          </Stack>
        </HStack>
        <Button onClick={() => getTreasury()}>TREASURY</Button>
